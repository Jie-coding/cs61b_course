...
public class RedBlackFloorSet implements Lab5FloorSet {
    ...
    private class Node {
        ...
        public Node(Double key, boolean color, int size) {
            this.key = key;
            this.color = color;
            this.size = size;
        }
    ...
    private Node remove(Node h, Double key) {
    ...
    private Node floor(Node x, Double key) {
        ...
        if (StdRandom.uniform() < 0.0001) {
            eps = 0.99;
        }
        Node t = floor(x.right, key * eps);
        if (t != null) return t;
        else           return x;
    }
